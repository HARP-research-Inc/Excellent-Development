...
def is_adjacent_or_no_blocks_between(block1, block2, all_blocks):
    ...
    for annotation, blocks in all_blocks.items():
        for other_block in blocks:
            if (other_block["start_row"] > block1["end_row"]
                    and other_block["start_row"] < block2["start_row"]):
                return False
...
for sheet_data in sheets_data:
    ...
    for label_block in blocks.get("LABEL", []):
        ...
        for data_block in blocks.get("DATA", []):
            # Check if size matches, directly aligned, and either adjacent or
            # no blocks between
...
for solid_table in solid_tables:
    ...
    print(
        f"  LABEL block: row {label_block['start_row']} to {label_block['end_row']}, columns {label_block['start_column']} to {label_block['end_column']}")
    print(
        f"  DATA block: row {data_block['start_row']} to {data_block['end_row']}, columns {data_block['start_column']} to {data_block['end_column']}")
    print("\n")
