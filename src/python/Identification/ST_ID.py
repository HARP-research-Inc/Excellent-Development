    ...
    if 'blocks' in sheet:
        for block_type in sheet['blocks']:
            for block in sheet['blocks'][block_type]:
                ...
                block['start_column'] = excel_column_number(
                    block['start_column'])
                block['end_column'] = excel_column_number(block['end_column'])
                # calculate the size of the block
                block['x_size'] = block['end_column'] - \
                    block['start_column'] + 1
                block['y_size'] = block['end_row'] - block['start_row'] + 1
                block['pos'] = {
                    'start': [
                        block['start_column'], block['start_row']], 'end': [
                        block['end_column'], block['end_row']]}
...
def initialize_tabulate_and_export(
        input_filename="JSONs/blocks_output.json",
        output_filename="JSONs/solid_table_output.json",
        debug=True):
    # Load json data
