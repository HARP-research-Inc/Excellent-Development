...
def label_visualize():
    ...
    for item in data:
        ...
        for cell_location, cell_info in annotations.items():
            ...
            cell = sheet.cell(row=int(cell_location[1:]), column=ord(cell_location[0]) - 64)
            cell.value = value
...
def solid_block_visualize():
    ...
    def add_data_to_sheet(ws, block):
        for cell, details in block['cells'].items():
            ...
            cell.fill = green_fill if details['annotation'] == 'LABEL' else (red_fill if details['annotation'] == 'DATA' else orange_fill)
    ...
    for sheet in data:
        ...
        for annotation_type, blocks in sheet['blocks'].items():
            for i, block in enumerate(blocks, start=1):
                ws = wb.create_sheet(title=f"Sheet {sheet['sheet_number']} {annotation_type} {i}")
    ...
    # Open the Excel file
    os.system('start excel.exe "{}"'.format('visuals/solid_block_visualized.xlsx'))
...
def solid_table_visualize():
    ...
    wb.save(filename=f'visuals/output_table_visualized_sheet.xlsx')
    
    # Open the spreadsheet in Excel
    os.system('start excel.exe "{}"'.format(f'visuals/output_table_visualized_sheet.xlsx'))
